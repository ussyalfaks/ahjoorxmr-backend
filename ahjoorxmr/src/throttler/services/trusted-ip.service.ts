...
export class TrustedIpService {
  ...
  constructor(
    ...
    this.trustedIps = new Set(
      trustedIpsEnv.split(',').map(ip => ip.trim()).filter(Boolean),
    );

    // Load trusted IP ranges (CIDR notation or start-end ranges)
    const trustedRangesEnv = this.configService.get<string>('TRUSTED_IP_RANGES', '');
    ...
    if (this.trustedIpRanges.length > 0) {
      this.logger.log(`Loaded ${this.trustedIpRanges.length} trusted IP ranges`);
    }
  }
  ...
  isTrustedIp(ip: string): boolean {
    ...
    for (const range of this.trustedIpRanges) {
      if (this.isIpInRange(ip, range.start, range.end)) {
        this.logger.debug(`IP ${ip} is in trusted range ${range.start}-${range.end}`);
        return true;
      }
    }
  ...
  async blockIp(ip: string, duration: number, reason?: string): Promise<void> {
    ...
    this.logger.warn(`Blocked IP ${ip} for ${duration}s. Reason: ${reason || 'Rate limit exceeded'}`);
  }

  /**
   * Check if an IP is currently blocked
   */
  async isIpBlocked(ip: string): Promise<{ blocked: boolean; reason?: string }> {
    const key = `blocked_ip:${ip}`;
   ...
   */
  async getBlockedIps(): Promise<Array<{ ip: string; reason: string; ttl: number }>> {
  ...
  async incrementViolations(
    ...
    if (shouldBlock) {
      await this.blockIp(ip, 3600, `Exceeded ${threshold} violations in ${windowSeconds}s`);
      this.logger.warn(`IP ${ip} exceeded violation threshold (${count}/${threshold})`);
    }
  ...
  private isIpInRange(ip: string, start: string, end: string): boolean {
    if (!this.isValidIpv4(ip) || !this.isValidIpv4(start) || !this.isValidIpv4(end)) {
      return false;
    }
  ...
  async getIpInfo(ip: string): Promise<{
    ...
    const violations = parseInt(await this.redis.get(violationsKey) || '0', 10);
